...
It uses an isolated temp workspace and exercises wallet creation, Friendbot funding, XLM payment, bundled Testnet payments, ledger lookups, receipt verification, report export, issued-asset payment after trustline setup, Blend deployment discovery, Blend trustline guidance, Blend Testnet supply and batch transactions, core liquidity-pool list/inspect/trades/position/listener/LP preflight checks, signed-XDR approval submission, trustline add/remove, XLM and issued-asset claimable balance create/claim, approval-gated payment, local x402, local one-time MPP, local MPP session budget, and Stellar CLI contract asset deployment/read/info/invocation/TTL extension when `stellar` is installed.

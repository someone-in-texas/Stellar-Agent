...
GitHub releases package this plugin as `stellar-agent-codex-plugin-v0.4.3.tgz` alongside the npm tarball for `@stellar-agent/codex-plugin`. The release preflight validates the staged plugin artifact with the packaged validator so the Codex plugin and GitHub release remain aligned.
